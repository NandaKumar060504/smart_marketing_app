...
col2.metric("Clicks", (df["action"] == "clicked").sum())
total_revenue = df[df["action"] == "purchased"]["price"].sum()
...
# Add Revenue per Ad
revenue_per_ad = df[df["action"] == "purchased"].groupby("ad_id")["price"].sum()
ad_summary["Revenue"] = revenue_per_ad
ad_summary = ad_summary.fillna(0)

st.dataframe(
    ad_summary.style.format({
        "CTR": "{:.2%}",
        "Conversion Rate": "{:.2%}",
        "Revenue": "₹{:.0f}"
    })
